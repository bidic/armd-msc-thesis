...
__inline void AT45_WriteBuffer(At45* pAt45,short int siBuffNr,unsigned char* pBuff, unsigned int uiSize)
{
  while(AT45_SendCommand(pAt45, (siBuffNr == 0) ? AT45_BUF1_WRITE : AT45_BUF2_WRITE, 4, pBuff, uiSize, 0, 0, 0))
  {
...
unsigned char AT45_EraseChip(At45* pAt45)
{
  ...
  unsigned char error = AT45_SendCommand(pAt45, opcode, 1, opcodes, 3, 0, 0, 0);
...
unsigned char AT45_FastWrite(At45* pAt45,short int siBuffNr,unsigned char* pBuff, unsigned int uiSize, unsigned int uiAddress)
{
  unsigned char cmd1;
  unsigned char cmd2;
  ...
  unsigned char error = AT45_SendCommand(pAt45, cmd1, 4, pBuff, uiSize, 0, 0, 0);
...
__inline void AT45_BufferToMem(At45* pAt45,short int siBuffNr, unsigned int uiAddress)
{
  AT45_SendCommand(pAt45, (siBuffNr == 0) ? AT45_BUF1_MEM_NOERASE : AT45_BUF2_MEM_NOERASE, 4, 0, 0, uiAddress, 0, 0);
...
unsigned char AT45_EraseBlock(At45* pAt45,unsigned int uiAddress)
{
  ...
  unsigned char error = AT45_SendCommand(pAt45, opcode, 4, 0, 0, uiAddress, 0, 0);
...
//  AT91C_BASE_PMC->PMC_PLLR = 1 | 1 << 16;
  ...
  AT91F_PMC_EnablePCK(AT91C_BASE_PMC, 1, AT91C_PMC_CSS_MAIN_CLK | AT91C_PMC_PRES_CLK_64); //MAIN_CLK podzielony przez 64

  // KONFIGURACJA pinow SPI dla pamieci flash oraz PA21 dla MCK kamery
  AT91F_PIO_CfgPeriph( AT91C_BASE_PIOA, AT91C_PA12_MISO | AT91C_PA13_MOSI | AT91C_PA14_SPCK | AT91C_PA31_NPCS1, AT91C_PA21_PCK1);
  ...
  {
    sprintf(txt,"Test: %s page size %d",pDesc->name, AT45_PageSize(&sAt45));
    printf(txt);
  }
  printf("poszlo");
  int i=0;
