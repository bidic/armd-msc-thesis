...
// RS-A4
// EN-A5
#define LCDRS(arg)  if(arg==1) lcdBuff=lcdBuff | 8; else if(arg==0) lcdBuff= ~(~lcdBuff | 8); TWID_Write(&twid, AT24C_ADDRESS, 0, 0, &lcdBuff, 1, 0);
#define LCDE(arg)   if(arg==1) lcdBuff=lcdBuff | 4; else if(arg==0) lcdBuff= ~(~lcdBuff | 4); TWID_Write(&twid, AT24C_ADDRESS, 0, 0, &lcdBuff, 1, 0);
#define LCDD4(arg)  if(arg==1) lcdBuff=lcdBuff | 16;  else if(arg==0) lcdBuff= ~(~lcdBuff | 16);  TWID_Write(&twid, AT24C_ADDRESS, 0, 0, &lcdBuff, 1, 0);
#define LCDD5(arg)  if(arg==1) lcdBuff=lcdBuff | 32;  else if(arg==0) lcdBuff= ~(~lcdBuff | 32);  TWID_Write(&twid, AT24C_ADDRESS, 0, 0, &lcdBuff, 1, 0);
#define LCDD6(arg)  if(arg==1) lcdBuff=lcdBuff | 64;  else if(arg==0) lcdBuff= ~(~lcdBuff | 64);  TWID_Write(&twid, AT24C_ADDRESS, 0, 0, &lcdBuff, 1, 0);
#define LCDD7(arg)  if(arg==1) lcdBuff=lcdBuff | 128;  else if(arg==0) lcdBuff= ~(~lcdBuff | 128);  TWID_Write(&twid, AT24C_ADDRESS, 0, 0, &lcdBuff, 1, 0);
