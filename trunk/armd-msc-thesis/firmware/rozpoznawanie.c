...
////////////////////////////////////////////////////////////////////////////////
extern float wzorzec1, wzorzec2, wzorzec3, wzorzec4, wzorzec5, wzorzec6, wzorzec7, wzorzec8, wzorzec9;
...
inline int Rozpoznaj()
{
  ...
  for(wsk=0; wsk<64000; ++wsk )
  {
    if(mem[wsk] > srednia_255) {mem[wsk] = 255;}
      else {mem[wsk] = 0;}
  }
  ...
  for(wsk=0; wsk<64000; ++wsk )
  {
    if(mem[wsk] > 0) { SetPx2bit(wsk, 1); }
      else { SetPx2bit(wsk, 0); }
  }
  ...
  if(AutoPreview)
  {
    ...
    mem[47992]='p'; mem[47993]=1;
    UART0_DMA_Write(&mem[47992], 8);//wypelniacz - dane bez sensu
  }


  x=0; y=0; ObszarCounter=0; ZnalezionoObszar=0;
  while (y<200)
  {
    ...
    if ( GetPx2bit(ToWsk(x,y)) == 0 )
    {
      ...
      if( SmithFill(x, y, 0, 3) > 100 )
      {
        ...
        ZnalezionoObszar=1;
        ++ObszarCounter;
      } else {SmithFill(x, y, 3, 1);}
    }

    //Dla kazdego znalezionego obszaru spójnego analiza i rozpoznawanie
    if (ZnalezionoObszar)
    {
      ...
      wspolczynnik1=0; 
      if(odleglosc!=0) { wspolczynnik1 = ((float)pole / odleglosc)*2.0; }
      ...
      if(xmin>1 && ymin>1 && xmax<318 && ymax<198)
      {
        for(i=xmin-1; i<xmax+1; ++i)
        {
          for(j=ymin-1; j<ymax+1; ++j)
          {
            ...
            if(GetPx2bit(ToWsk(i,j)) == 2 && GetPx2bit(ToWsk(i+1,j)) != 2)
            { 
              ++rzut1;
            }

            //dlugosc rzutu 45stopni
            if(GetPx2bit(ToWsk(i,j)) == 2 && GetPx2bit(ToWsk(i+1,j-1)) != 2)
            { 
              ++rzut2;
            }

            //dlugosc rzutu 90stopni
            if(GetPx2bit(ToWsk(i,j)) == 2 && GetPx2bit(ToWsk(i,j-1)) != 2)
            { 
              ++rzut3;
            }
      ...
      if(xwzorzec>0 && ywzorzec>0 && GetPx2bit(ToWsk(xwzorzec,ywzorzec)) == 2)
      {
        if (WzorzecCount==0)
        {
        ...
        if (WzorzecCount==3){ WzorzecCount=0;}
      }
      ...
      {
        tmp = sqrt( ((wspolczynnik1-wzorzec1)*(wspolczynnik1-wzorzec1)) + ((wspolczynnik2-wzorzec2)*(wspolczynnik2-wzorzec2)) 
        + ((wspolczynnik3-wzorzec3)*(wspolczynnik3-wzorzec3)) );
        if( tmp < roznica) { roznica = tmp; RozpoznanyX = mx; RozpoznanyY = my;}
      }

      if(wzorzec4>0 && wzorzec5>0 && wzorzec6>0)
      {
        tmp = sqrt( ((wspolczynnik1-wzorzec4)*(wspolczynnik1-wzorzec4)) + ((wspolczynnik2-wzorzec5)*(wspolczynnik2-wzorzec5)) 
        + ((wspolczynnik3-wzorzec6)*(wspolczynnik3-wzorzec6)) );
        if( tmp < roznica) { roznica = tmp; RozpoznanyX = mx; RozpoznanyY = my;}
      }

      if(wzorzec7>0 && wzorzec8>0 && wzorzec9>0)
      {
        tmp = sqrt( ((wspolczynnik1-wzorzec7)*(wspolczynnik1-wzorzec7)) + ((wspolczynnik2-wzorzec8)*(wspolczynnik2-wzorzec8)) 
        + ((wspolczynnik3-wzorzec9)*(wspolczynnik3-wzorzec9)) );
        if( tmp < roznica) { roznica = tmp; RozpoznanyX = mx; RozpoznanyY = my;}
      }
  ...
  //sterowanie serwem
  if( RozpoznanyY > 120) {SerwoAngleBuffer = SerwoAngleBuffer + 10;}
  if( RozpoznanyY < 80) {SerwoAngleBuffer = SerwoAngleBuffer - 10;}
