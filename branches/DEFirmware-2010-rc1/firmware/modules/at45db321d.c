    ...
    unsigned char* pBuff, unsigned int uiSize, unsigned int uiAddress)
{
  ...
  unsigned char error =
      AT45_SendCommand(pAt45, cmd1, 4, pBuff, uiSize, 0, 0, 0);
...
unsigned char AT45_EraseBlock(At45* pAt45, unsigned int uiAddress)
{
  ...
  unsigned char error = AT45_SendCommand(pAt45, opcode, 4, 0, 0, uiAddress, 0,
      0);
...
void AT45DB321D_SelfTest()
{
  ...
  for (i = 0; i < 480; i++)
  {
      ...
      if (wsk & 0x200)
      {
        ...
        if ((pAt45->pSpid->pSpiHw->SPI_SR & AT91C_SPI_RXBUFF))
        {
          ...
          unsigned char error = AT45_SendCommand(pAt45,
              (!bufSel) == 0 ? AT45_BUF1_MEM_NOERASE : AT45_BUF2_MEM_NOERASE,
              4, 0, 0, row++ << 9, 0, 0);
    ...
    if ((pAt45->pSpid->pSpiHw->SPI_SR & AT91C_SPI_RXBUFF))
    {
      ...
      unsigned char error = AT45_SendCommand(pAt45,
          (!bufSel) == 0 ? AT45_BUF1_MEM_NOERASE : AT45_BUF2_MEM_NOERASE, 4, 0,
          0, row++ << 9, 0, 0);
      ...
      AT45_SendCommand(pAt45,
          (bufSel == 0) ? AT45_BUF1_WRITE : AT45_BUF2_WRITE, 4, mem
              + (last_idx_written * PAGE_SIZE), 512, 0, 0, 0);
