...
__ramfunc void SYSTEM_INTERRUPT_irq_handler(void)
{
  ...
  unsigned int dummy;
  Pin pin_serwo_pwm = PIN_SERWO_PWM;
...
int main(void)
{
  ...
  HY1602F6_Log("Calibrating","gyroscope ...");
  L3G4200D_Calibrate(&twid);

//  Configure_DarkExplorer_Pins();

  HY1602F6_Log("Preparing camera","memory ...");
    ...
    switch (FrameSizeToGet)
    {
      ...
      case 6://160x120 b&w
      {
        ...
        while (it < iAmountOfPackets)
        {
          ...
          AT45DB321D_Read(pic_buff, 1280, 1280 * it + 512);
          waitms(150);
          it++;
          if (!USART_WriteBuffer(AT91C_BASE_US0, pic_buff, 1280))
          {
            it--;
          }
        }
        ...
        while (mag_enable)
        {
          ...
          if (++iter % 100 == 0)
          {
            ...
            sprintf(lcd_buff1, "x: %3d y: %3d", (int) current_mg_i.x,
                (int) current_mg_i.y);
            sprintf(lcd_buff2, "angle: %3d", (int) cangle(current_mg_i.x,
                current_mg_i.y));
