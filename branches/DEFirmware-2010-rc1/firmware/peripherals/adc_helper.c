...
volatile unsigned int adc_channel_a;
volatile unsigned int adc_channel_b;
volatile unsigned int adc_channel_c;
volatile unsigned int read_channels = 0;
...
static void ADC_TripleChannelInterruptHandler(void)
{
  ...
  if (read_channels >= 3)
  {
    tripleChannelCallback(channel_a_output, channel_b_output, channel_c_output);
    adc_data_ready = 1;
  }
}
