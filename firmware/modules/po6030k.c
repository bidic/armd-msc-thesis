...
void PO6030K_InitRegisters(Twid *twid) {
	...
	// 640x480 color
	unsigned int reg[] = { 0x03, 0x91, 0x03, 0x38, 0x03, 0x68, 0x03, 0x80,
			0x03, 0x81, 0x03, 0x51, 0x03, 0x53, 0x03, 0x54, 0x03, 0x55, 0x03,
			0x56, 0x03, 0x57 };
	unsigned char comm[] = { 0x00, 0x30, 0x01, 0x00, 0x01, 0x00, 0x01, 0x20,
			0x01, 0x20, 0x01, 0x07, 0x01, 0x07, 0x01, 0x02, 0x01, 0x86, 0x01,
			0x01, 0x01, 0xE7 };
...
inline void AT45DB321D_SendCommand(unsigned char cmd, unsigned char cmdSize,
		unsigned char *pData, unsigned int dataSize, unsigned int address) {
	...
	AT91C_BASE_SPI->SPI_MR = ((AT91C_BASE_SPI->SPI_MR | AT91C_SPI_PCS) & (~(1
			<< 17)));
...
void PO6030K_TakePicture() {
	...
	while (GET_VSYNC) //podczas gdy VSYNC == 1
	{
		...
		while (GET_HSYNC && GET_VSYNC) //podczas gdy HSYNC == 1
		{
			...
			if (wsk & 0x200) {
				...
				AT45DB321D_SendCommand(
						!buf ? AT45_BUF1_WRITE : AT45_BUF2_WRITE, 4, mem
								+ (PREV_BUFF_IDX(idx) * PAGE_SIZE), PAGE_SIZE,
						0);
	...
	AT45DB321D_SendCommand(buf ? AT45_BUF1_MEM_NOERASE : AT45_BUF2_MEM_NOERASE,
			4, 0, 0, row++ << 9);
	RELEASE_SPI_WHEN_READY(pSpid);

	AT45DB321D_SendCommand(!buf ? AT45_BUF1_WRITE : AT45_BUF2_WRITE, 4, mem
