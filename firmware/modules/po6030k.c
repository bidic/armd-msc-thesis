...
//pamiec obrazu
char mem[MAX_BUFF_IDX * PAGE_SIZE * 2] = {0};
extern Twid twid;
...
void PO6030K_InitRegisters(Twid *twid)
{
  ...
  // 640x480 color
  unsigned int reg[] = { 0x03, 0x91, 0x03, 0x38, 0x03, 0x68, 0x03, 0x80, 0x03,
      0x81, 0x03, 0x51, 0x03, 0x53, 0x03, 0x54, 0x03, 0x55, 0x03, 0x56, 0x03,
      0x57 };
  unsigned char comm[] = { 0x00, 0x30, 0x01, 0x00, 0x01, 0x00, 0x01, 0x20,
      0x01, 0x20, 0x01, 0x07, 0x01, 0x07, 0x01, 0x02, 0x01, 0x86, 0x01, 0x01,
      0x01, 0xE7 };
    ...
    while (GET_HSYNC && GET_VSYNC) //podczas gdy HSYNC == 1
    {
      ...
      if (wsk & 0x200)
      {
        ...
        AT45DB321D_SendCommand(!buf ? AT45_BUF1_WRITE : AT45_BUF2_WRITE, 4, mem
            + (PREV_BUFF_IDX(idx) * PAGE_SIZE), PAGE_SIZE, 0);
