...
\begin{figure}[ht!]
 \centering
 \includegraphics[height=75mm]{../images/ch03/Eclipse-CPP-Project.jpg}
...
zgodna z nazwą katalogu zawierającego kod robota. Następnie z listy Project
type, wybieramy \textit{Makefile project --> Empty Project}, a jako \textit{Toolchain}
wybieramy \textit{Other toolchain}. Całą operację zatwierdzamy przyciskiem
\textit{Finish}.

\begin{figure}[ht!]
 \centering
 \includegraphics[width=145mm]{../images/ch03/Eclipse-MainWindow.jpg}
...
odpowiedniego toolchain'a w zmiennych środowiskowych systemu należy podjąć
dodatkowe kroki mające na celu uzupełnienie brakującej konfiguracji. Eclipse
umożliwia konfigurowanie tych zmiennych dla pojedynczych projektów. W celu ustawienia wymaganej ścieżki klikamy prawym klawiszem myszy na
nazwie naszego projektu w \textit{Project Explorer} i~wybieramy
\textit{Properties}.
\begin{figure}[ht!]
 \centering
 \includegraphics[width=\textwidth]{../images/ch03/Eclipse-Project-Properties.jpg}
 \caption{Okno dialogowe Properties}
 \label{fig:Eclipse-Project-Properties}
\end{figure}

Po ukazaniu się okna dialogowego \textit{Properties} (rys. \ref{fig:Eclipse-Project-Properties}) wybieramy \textit{C/C++ Build -->
...
katalogu, na potrzeby tej pracy załóżmy że będzie to katalog \url{/usr/local/}.
W celu zapewnienia dostępu do toolchain'a wszystkim programom wskazane jest
dodanie ścieżki /usr/local/bin do zmiennej środowiskowej PATH (komenda 
\verb|export PATH=$PATH:/usr/local/bin|). W celu sprawdzenia poprawności instalacji należy wykonać komendę:
\begin{verbatim}
arm-elf-gcc --version 
\end{verbatim}
...
\begin{figure}[ht!]
 \centering
 \includegraphics[width=\textwidth]{../images/ch03/arm-elf-gcc-test.jpg}
...
On--Chip Debugger. Źródła programu należy pobrać z oficjalnej strony projektu:
\url{http://sourceforge.net/projects/openocd/}. Autorzy programu nie zamieścili
wersji binarnych, więc kompilacje będziemy musieli przeprowadzić sami.
...
\end{verbatim}
W wyniku wykonania polecenia na ekranie powinniśmy otrzymać widok podobny do
tego który przedstawiony jest na rysunku \ref{fig:openocd}.
\begin{figure}[ht!]
 \centering
 \includegraphics[width=0.8\textwidth]{../images/ch03/openocd.jpg}
...
openocd w celu podłączenia się do robota. Następnie uruchamiamy program telnet
za pomocą polecenia:
